...
import 'package:zefyr_apex/util.dart';
...
class ZefyrController extends ChangeNotifier {
  ...
  List<String> get triggers => List.from(_mentionTriggers);
...
//    _lastChangeSource = source;
  ...
  void _checkForMentionTriggers() {
    ...
    if (isMentionSubmitted.contains(NotusAttribute.mentionPerson) ||
        isMentionSubmitted.contains(NotusAttribute.mentionPost) ||
        isMentionSubmitted.contains(NotusAttribute.mentionTopic)) {
      return;
    }
    ...
    if (plainText.substring(
            ...
            ':' &&
        getSelectionStyle().containsSame(NotusAttribute.block.code)) {
      return;
    }
    if (plainText.substring(
                indexOfLastMentionTrigger, indexOfLastMentionTrigger + 1) ==
