...
import 'package:zefyr_apex/src/widgets/baseline_proxy.dart';
  ...
  Widget build(BuildContext context) {
    return Hero(
      ...
      child: kIsWeb || !hasVideo
          ? _MessageWidget(hasVideo ? 'Tap to play video' : 'could not load load video!')
          : connectionState == ConnectionState.done
              ? Container(
                  alignment: data.align.isCenter
                      ? Alignment.center
...
Widget ZefyrAudio(EmbedNode node) {
  ...
  if (!lookupMimeType(path).isAudio) return _MessageWidget('could not load audio');
              ...
              ? AspectRatio(
                  ...
                  child: Stack(
                    ...
                    children: <Widget>[
                      ...
                      Align(alignment: Alignment.topCenter, child: _buildSpeedAndClose()),
                      Align(
                        ...
                        child: VideoProgressIndicator(
                          ...
                          colors: VideoProgressColors(
                              backgroundColor: Colors.grey, playedColor: Colors.blue),
                        ),
                      ),
                    ],
                  ),
...
class ZefyrEditor extends StatefulWidget {
  ...
  final Future<List<Suggestions>> Function(String trigger, String value) suggestionListBuilder;
...
class _ZefyrEditorSelectionGestureDetectorBuilder
  ...
  void _handleEmbedsClick(TapUpDetails details) {
    ...
    if (line.hasEmbed) {
      ...
      if (node.value.type == 'video') {
        ...
        final path = node.value.data['source'] ?? data.localPath ?? data.url ?? '';
      ...
      if (node.value.type == 'image') {
        final data = ImageData.fromJson(node.value.data);
        final path = node.value.data['source'] ?? data.localPath ?? data.url ?? '';
        final hasImage = lookupMimeType(path).isImage;
        if (hasImage) {
          if (editor.widget.readOnly) {
            _openImage(path);
          } else {
  ...
  void _openImage(String path) {
    if (editor.widget.onImageClick != null) {
      editor.widget.onImageClick(path);
    } else {}
  }
  ...
  void _handleTextSegmentClick(TapUpDetails details) {
    ...
    if (segment.style.contains(NotusAttribute.link) && editor.widget.onLaunchUrl != null) {
      ...
      if (editor.widget.readOnly) {
        editor.widget.onMentionClicked
            ?.call(segment.style.get(NotusAttribute.mentionPerson).value, segment.toPlainText());
      }
    } else if (segment.style.contains(NotusAttribute.mentionPost)) {
      if (editor.widget.readOnly) {
        editor.widget.onMentionClicked
            ?.call(segment.style.get(NotusAttribute.mentionPost).value, segment.toPlainText());
      }
    } else if (segment.style.contains(NotusAttribute.mentionTopic)) {
      if (editor.widget.readOnly) {
        editor.widget.onMentionClicked
            ?.call(segment.style.get(NotusAttribute.mentionTopic).value, segment.toPlainText());
      }
    }
  }
}
...
//    _textChangedSinceLastCaretUpdate = true;
  ...
  void _updateOrDisposeSuggestionOverlayIfNeeded() {
    if (widget.controller.isInMentioningMode && widget.suggestionListBuilder != null) {
      SchedulerBinding.instance.addPostFrameCallback((Duration _) {
        ...
        if (widget.controller.mentionTrigger == ':') {
          ...
          _suggestionOverlay = MentionSuggestionOverlay(
              ...
              suggestionListBuilder: Future.value(programmingLanguageSuggestion),
        ...
        if ((controller.mentionedText.contains(' ')) && controller.mentionTrigger != ':') {
          _handleMentionSuggestionSelected(-1, controller.mentionTrigger + controller.mentionedText,
  ...
  void _handleMentionSuggestionSelected(int key, String value, {bool selectedFromList = true}) {
    final controller = widget.controller;

    final mentionStartIndex = controller.selection.end - controller.mentionedText.length - 1;
    ...
    controller.replaceText(mentionStartIndex, mentionedTextLength, replacementText,
        selection: TextSelection.collapsed(offset: mentionStartIndex + replacementText.length));
    ...
    if (controller.mentionedText.length > 1) {
      controller.formatText(mentionStartIndex, replacementText.length - 1, notus);
    } else {
